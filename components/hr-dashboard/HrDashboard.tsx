...
import { Users, FileCheck, UsersRound, ArrowRight, BarChart3 } from "lucide-react";
...
const HrDashboard = () => {
    ...
    const chartData = stats ? [
        ...
        { name: 'Pending', count: stats.pendingHrVerificationCount, color: '#ef4444' },
    ] : [];

    if (loading) return <div className="p-8 text-gray-500 animate-pulse">Loading HR Insights...</div>;
        ...
        <div className="p-8 bg-gray-50 min-h-screen space-y-8">
            ...
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-gray-500">Manage organizational compliance and employee records.</p>
                </div>
            </div>

            {/* Stat Cards Row */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card hoverable className="cursor-pointer" onClick={() => router.push('/hr/hr-reimbursement-request')}>
                    <Card.Content className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h2 className="text-2xl font-bold text-gray-900">{stats?.pendingHrVerificationCount || 0}</h2>
                        </div>
                    </Card.Content>
                </Card>
                ...
                <Card hoverable className="cursor-pointer" onClick={() => router.push('/hr/events')}>
                    <Card.Content className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Team Events Pending</p>
                ...
                <Card>
                    <Card.Content className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h2 className="text-2xl font-bold text-gray-900">{stats?.totalEmployees || 0}</h2>
                        </div>
                    </Card.Content>
                </Card>
            </div>
                    ...
                    <Card.Content>
                        <div className="space-y-4">
                            {stats?.pendingHrVerificationCount === 0 ? (
                                <p className="text-center py-6 text-gray-400 italic">No pending items.</p>
                            ) : (
                                Object.keys(stats?.pendingByType || {}).map((type) => (
                                    <div key={type} className="flex justify-between items-center p-3 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                                        <div>
                                            <p className="font-semibold text-gray-900 text-xs uppercase tracking-wider">{type.replace("_", " ")}</p>
                                            <p className="text-xs text-gray-500">{stats?.pendingByType?.[type] || 0} Items</p>
                                        </div>
                                        <Button 
                                            onClick={() => router.push(`/hr/hr-reimbursement-request`)}
